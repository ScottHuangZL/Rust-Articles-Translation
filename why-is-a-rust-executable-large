#为什么Rust可执行程序巨大
[原文链接](
https://lifthrasiir.github.io/rustlog
/why-is-a-rust-executable-large.html)
...
```
这实际上并没有什么区别！这是因为优化仅仅处理了用户代码，而我们机会没有写什么用户代码。几乎所有的二进制代码来自标准库，而且我们不清楚可以做些啥...

**链接时间优化(LTO)**

…可惜我们可以进入链接时间优化。

因此故事就如下面：我们可以分别优化每一个crate，实际上所有标准库已经用优化形式发布。一旦编译器产生一个优化后的二进制文件，它被"the linker"组装成一个单独的可执行文件。但我没不需要整个标准库：举例，一个简单的"Hello, workd"程序当然不需要std::net库。然而，链接器这么笨以至于它不会尝试移除crates中没用到的部分；它仅仅把这些库贴到最后的可执行文件中。

其实传统linker这么做有一个好的理由。链接器通常用在C和C++，而且每个文件单独编译。这和Rust有很大的不同，因为整个crate是一起编译的。除非需要的函数分布在各个文件中，链接器可以相当容易的一次性去除一些没有用到的文件。这不完美，但基本接近我们的需要：移除没有用到的函数。一个不利的地方是编译器没有办法优化函数调用当它指向其他文件时；简单来说是缺少需要的信息。

C和C++伙计们已经适应这个大约几十年了，但在最近的一二十年，他们觉得受够了，并且开始提供一个选项来启用链接时间优化(link-time optimization LTO)在这个计划里，编译器为每个文件产生优化的二进制代码而没有瞧其它，然后链接器积极的看所有的东西并且尝试优化最终的二进制文件。这比直接处理（内部简化）源文件要难多了，而且这使编译的时间大大变长了，但又值得一试如果必须获得一个更小的且/或更快的可执行文件。
...
Binutils有许多工具，但strings也许是最简单的一个。它简单的爬行二进制文件来发现用0字节结尾的可打印字符序列，一个典型的C表达方式的string。这样它尝试从二进制文件中抽取可读的strings，这对我们很有帮助。所以，让我们试一下，且准备滚屏:
```
...
那些以backtrace_ 和 DW_开头的字符串，他们是从库 libbacktrace获得的。一个用于产生栈跟踪的库。 Rust用它来打印一些有用的panic跟踪信息(当 RUST_BACKTRACE=1 环境下有效)。然而，我们没有pacnic。

那些以_ZN开头的字符串,这些是“mangled”的名字，从Rust标准库中来。

为什么我们刚开始有这些strings？它们是调试符号，相对于机器处理的二进制文件，这些合适的名字大概人类还可以阅读一下。你还记得上面提到的libbacktrace？它需要这些调试符号来打印出任何有用的信息。然而，因为我们确实想要发布一个release版，我们可以选择不包含调试信息。Rust本身没有这个选项，因为他们一般是利用一个外部工具叫做strip。因此，让我们来瞧一下用它如何工作。
...
```
我们已经搞定调试符号了，现在让我们去除剩下的库。一些坏消息：从这个点开始，你进入了nightly rust领域。这个领域并没有你想的那么可怕，因为它并没有打你脸，但它会在一些小地方打击你（这也是为什么我们叫他nighlies!）。这也是为啥我们没有nightly特性在stable版本里的主要理由--它们也许会改变。幸运的是，我们下面在用的那些特性已经相当稳定了，而且你也许可以跟踪这篇文章来得知更多的nightlies。但之后，我会坚持一个特定的nightly版本。
...
```
(译者现在编译出来的程序是147K，比先前的170K有减少了一些)
好的，看起来Rust实际上远比C和C++的编译效果好。但是...为啥它是“公平”的？不管任何语言，难道一个这么简单的程序需要正么多空间吗？
...
**如果你的程序没有密集用到内存管理，请用 system allocator (假定你用 nightly版本).**

**你也许可以在未来使用编译参数开关 s/z**
